...
{
    const RV v = std::move(stack.back());
    stack.pop_back();
    return v;
}
...
wave::internal::RV wave::internal::VM::add(const RV& a, const RV& b)
{
    ...
    std::cerr << "RuntimeError: Cannot apply '+' to operands of the given types." << "\n";
    exit(1);
...
void wave::internal::VM::print()
{
    ...
    if (std::holds_alternative<std::shared_ptr<std::string>>(arg)) {
        std::cout << *std::get<std::shared_ptr<std::string>>(arg) << "\n";
        return;
    }
    ...
    if (std::holds_alternative<bool>(arg)) {
        std::cout << std::boolalpha << std::get<bool>(arg) << "\n";
        return;
    }
}
...
void wave::internal::VM::load_const(double index)
{
    ...
    {
        std::cout << "Runtime Error: Constant pool access failed. The provided index does not reference an existing constant." << "\n";
        exit(1);
    }
